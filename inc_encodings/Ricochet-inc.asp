...
assumption_init(pos(R,1,I,0),false) :- not pos(R,I,_), domain_pos(R,1,I).

assumption_init(pos(R,-1,J,0), true) :-     pos(R,_,J).
assumption_init(pos(R,-1,J,0),false) :- not pos(R,_,J), domain_pos(R,-1,J).

% goal
assumption_goal(pos(R,1,I), true) :- target(R,I,_).
assumption_goal(pos(R,-1,J), true) :- target(R,_,J).

domain_pos(R,O,IJ) :- robot(R), domO(O), dim(IJ).
domain_go(R,D,O) :- robot(R), dir(D,O).
...
{pos(R,O,IJ,0) : domain_pos(R,O,IJ)}.  
...
blocked(R,D,O,I+DI,t) :- go(R,D,O,t), pos'(R,-O,L,t), not conn(D,L,I), dl(D,DI), dim(I), dim(I+DI).
...
:- go(R,D,O,t), pos'(R,O,I,t), blocked(R,D,O,I+DI,t), dl(D,DI).
:- go(R,D,O,t), go'(R,DD,O,t).

pos(R,O,I,t) :- reachable(R,D,O,I,t), not reachable(R,D,O,I+DI,t), dl(D,DI).
pos(R,O,I,t) :- pos'(R,O,I,t), not go_(R,O,t).
